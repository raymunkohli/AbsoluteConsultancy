        ...
        <img id="u519_img" class="img " src="images/receptionist_screen/u84.png">
            ...
            <div id="u479" class="ax_default">
                <table>
                    ...
                    <tr><th>Customer ID </th> <th>Customer Name</th><th>JobIDs </th> <th>Price</th><th>Order Date </th><th>Send Email Reminder</th><th>First Reminder</th><th>Second Reminder</th></tr>
                            <c:forEach items="${default}" var="cust" varStatus="Status"> 
                        <tr><td>${cust.customerID}</td><td>${cust.name} ${cust.surname}</td><td>${jobd[Status.index]}</td><td>£${jobValued[Status.index]}</td> <td>${orderDatesd[Status.index]}</td> <td><form action="sendSecondEmail" method="post"> <input type="hidden" name="name" value="${cust.name} ${cust.surname}"><input type="hidden" name="email" value="${cust.email}"><input type="submit" value="send"></form></td><td>
                    ...
                    <tr><th>Customer ID </th> <th>Customer Name</th><th>JobIDs </th> <th>Price</th><th>Order Date </th><th>Send Email Reminder</th><th>First Reminder</th> </tr>
                            <c:forEach items="${suspended}" var="cust" varStatus="Status">
                        <tr><td>${cust.customerID}</td><td>${cust.name} ${cust.surname}</td><td>${job[Status.index]}</td><td>£${jobValue[Status.index]}</td> <td>${orderDates[Status.index]}</td> <td><form action="sendFirstEmail" method="post"> <input type="hidden" name="name" value="${cust.name} ${cust.surname}"><input type="hidden" name="email" value="${cust.email}"><input type="submit" value="send"></form></td><td>
